...
## Análisis de parametros a parir de Metodos en R.
...
Observacion: los elementos de la columna 2 y 3 son tipo caracter y al comvertir todo el dataset a matrix se convertian todos los elementos a caracter.
...
parametros
```
2- se grafico con la funcion "qplot" los columnas 5 y 6 que corresponden a los datos de caudal de gas y caudal de petroleo respectivamente.

```{r}
qplot(parametros[ , 5], parametros[ , 6], main= "Caudal de Gas vs Caudal de Petroleo",xlab="Gas", ylab= "Po")
```

3- mediante los graficos de cajas con la funcion "boxplot" se comparo los diferentes caudales produccion.
...
4- A partir de la funcion "plot(sort..." se represento el caudal total de produccion de forma ordenada.

```{r}
plot (sort(parametros[ , 8]), main= "Caudal Total",xlab="Valores", ylab= "Caudal")
```

5- A partir de la funcion "plot" se represento el caudal total de produccion de la forma que se fueron tomando los registros.
...
6- se utizo la funcion head() para mostrar las primeras filas del conjunto de datos (parametros), como un data frame. y luego se realizo un histograma con los valores obtenidos.
...
7- a partir de la funcion summary() se obtiene un resumen estadistico, donde se genero un resumen para cada columna del conjunto de datos.

```{r}
summary(parametros)
```

8- La función colSums() utilizo para calcular la suma de las columnas del data frame y se obtuvo un vector final que contiene los diferentes resultados.
...
10- grafico de densidad, el cual puede aplicarse a las direntes columnas y en este ejemplo se usa en la culumna 7 que corresponde al caudal de agua.

```{r}
plot(density(parametros[ , 7]), main= "grafico de densidad", xlab = "presion", ylab = "cantidad")
```

11- grafico de histograma representa la distribución de datos numéricos. Consiste en un conjunto de barras adyacentes, donde la longitud de cada barra representa la frecuencia o la densidad de ocurrencia de valores dentro de un intervalo específico.

```{r}
hist(parametros[ , 2], breaks = 10, main= "Histograma", xlab = "presion", ylab = "cantidad")
```

12- otra forma de visualizacion de datos.
